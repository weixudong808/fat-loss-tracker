...
建档末尾**看样默认订阅**：按下方早报 prompt 规则+真实档案数据渲染一份明早样报（明天非训练日就渲染休息日版）给用户看，附一句"明早8点这份准时来，改时间或不想要随时说"，随即创建（默认 8:00，当场拒绝才不建）：
...
- 今天是训练日 → 筛 日期=今天 且 状态=待完成 的行，得 动作名称/组数/次数/重量kg
- 回收钩子：筛 日期=昨天 的行，存在状态=待完成 且 无状态=已完成 → 钩子命中（用户记过或昨天没排计划都不会触发）
...
- 回收钩子命中时：在营养目标行前加一句"昨天练了吧？把实际重量报我，我记上帮你排下次。"；未命中一字不加
...
带回收钩子（昨天计划未回收时，插在营养目标行前）：`昨天练了吧？把实际重量报我，我记上帮你排下次。`
