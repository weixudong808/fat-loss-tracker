...
5. **建档后注册引导（硬步骤；时机固定在建档播报之后，装机时不提——首装要快，征询放在用户已有数据、最能听懂价值的时刻。说清四件事：作用、耗时、配额、不装的后果）**，口径："咱们已经建完档了，现在已经可以用了。接下来建议把这个技能装进千问「自定义技能」——这是千问的功能，装了以后你随时开新对话都能用上它、接得上今天的档案。过程大概要 5 到 10 分钟，会占用你 20 个自定义技能名额里的 1 个。装不装你定：同意我现在就开始装；不同意，这个技能就只能在当前这个对话里用，以后想装随时跟我说'注册'。"同意 → 按 §10 注册（单回合一口气走完）；不同意 → 一句"好的，随时想装说一声"收住，不再追问
...
技能放 workspace/skills/ 后会话内即可用（自动发现）。要跨对话使用并出现在千问 App「自定义技能」界面，需正式注册，流程四步：inspect（查现有技能与配额，上限 20）→ 安全审核（发一次性令牌）→ commit（令牌 + 完整文件集 + 版本号）→ 成功标志 `loaded: true`（2026-09-03 真机一次过）。**注册征询时机固定在建档完成后（§1 第 5 步的口径：作用/耗时/配额/后果四件事说清，用户同意才走），装机时不提——首装要快**。数据持久化 2026-09-03 真机实测：注册后新开对话能读出前一对话建的完整档案（workspace 带了过去）——本地 xlsx 后端即可跨对话，暂不需要飞书；若日后新对话发现档案变空，再按「存储后端」配飞书表格做数据持久化兜底。
